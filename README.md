...
> **Note (2026-06-11):** Synthesis and improved-plan work now lives in `project/` (see `project/README.md` and `project/01-corpus-review.md`). That folder will migrate to a dedicated master location; documents there supersede planning artifacts here as they are produced.
...
- `site/`: public Clark website surface with the main argument, updates feed, project map, first node, evidence layer, document library, and hybrid public docs
...
The repo now contains a coherent CLARK operating structure: venture thesis, research system, business plan draft, pitch draft, agent prompts, task packets, templates, workflows, decision controls, and a public website surface in `site/` with top-level pages for updates, project mapping, the first node, the evidence layer, and the document library. The next high-value work is validating the beachhead customer, competitor analogs, corridor advantage, and incentive stack while extending the site with additional source-backed documents.
